...
const getLatestByPeriodCached = unstable_cache(
  async (metricKey: MetricKey) => getLatestByPeriod(metricKey),
  ["home-latest-by-period"],
  { revalidate: 3600, tags: ["home", "nav"] }
);
...
export default async function HomePage({
  ...
  const years = await getLatestByPeriodCached(metricKey);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {years.length === 0 ? (
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <div className="text-sm text-neutral-600 mt-1">
            Нажми <span className="font-medium">"Загрузить Excel"</span> и выбери
            файл выгрузки. После загрузки появятся года и месяцы.
          </div>
        </div>
      ) : (
      ...
      <div className="text-xs text-neutral-500">
        Подсказка: если добавили новую метрику в сервис — просто загрузите тот же
        Excel ещё раз, и данные допишутся в текущий снимок.
      </div>
    </div>
  );
}
