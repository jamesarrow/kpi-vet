...
async function getLatestByPeriod(metricKey: MetricKey) {
  ...
  const latest = new Map<string, { year: number; month: number; value: number; numerator: number; denominator: number }>();
  ...
  const years = new Map<number, Array<{ month: number; value: number; numerator: number; denominator: number }>>();
  for (const item of latest.values()) {
    if (!years.has(item.year)) years.set(item.year, []);
    years.get(item.year)!.push({ month: item.month, value: item.value, numerator: item.numerator, denominator: item.denominator });
  }
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0 space-y-3">
          <div className={tabWrap}>
            ...
            <span
              className={
                tabBase + " text-neutral-400 cursor-not-allowed select-none"
              }
          ...
          <h1 className="text-3xl font-semibold tracking-tight">Метрики ветклиники</h1>
                  ...
                  return (
                    ...
                    >
                      <div className="text-sm text-neutral-500">
                        {monthNameRu(p.month)} • M{p.month}.{y.year}
                      </div>
