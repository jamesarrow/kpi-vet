...
const getReportsCached = unstable_cache(
  ...
  ["reports-list"],
  { revalidate: 3600, tags: ["reports", "nav"] }
);
          ...
          <tbody>
            {reports.map((r) => (
              <tr key={r.id} className="border-b hover:bg-neutral-50">
                <td className="p-3 whitespace-nowrap">{new Date(r.uploadedAtISO).toLocaleString("ru-RU")}</td>
            ...
            {reports.length === 0 ? (
              <tr>
                <td colSpan={3} className="p-6 text-neutral-600">
                  Пока нет загрузок.
                </td>
              </tr>
            ) : null}
