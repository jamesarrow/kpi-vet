...
function deltaTone(metricKey: MetricKey, d: number) {
  // Для churn рост — плохо, падение — хорошо.
  const positiveMeansGood = metricKey !== "churn_rate";
  if (d > 0.00001) return positiveMeansGood ? "pos" : "neg";
  if (d < -0.00001) return positiveMeansGood ? "neg" : "pos";
  return "neu";
}
...
function churnParts(o: Overall) {
  if (!o) return { prev: 0, curr: 0, left: 0 };
  const prev = Math.max(0, Math.round(o.denominator));
  const left = Math.max(0, Math.round(o.numerator));
  const curr = Math.max(0, prev - left);
  return { prev, curr, left };
}
...
export function PeriodReportClient({
  ...
  // Дельту всегда показываем у более нового периода (A) относительно B
  const mainA = aOverall?.value ?? 0;
  const mainB = bOverall?.value ?? 0;
  const mainDelta = aOverall && bOverall ? mainA - mainB : null;

  const mainAnimated = useAnimatedNumber(mainA);
  ...
  const table = useMemo(() => {
    ...
    return keys
      .map((k) => {
        ...
        const delta = a && bb ? a.value - bb.value : null; // A - B
  ...
  const sortedTable = useMemo(() => {
    const rows = [...table];
    rows.sort((x, y) => {
      const dx = x.delta ?? (sortMode === "drops" ? 999999 : -999999);
      const dy = y.delta ?? (sortMode === "drops" ? 999999 : -999999);
      return sortMode === "drops" ? dx - dy : dy - dx;
    });
    return rows;
  }, [table, sortMode]);
  ...
  const aCh = churnParts(metricKey === "churn_rate" ? aOverall : null);
  const bCh = churnParts(metricKey === "churn_rate" ? bOverall : null);

  const compareLabel = compareActive ? monthLabel(compareMonth0, compareYear0) : "";
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-3">
          ...
          {compareActive && (
            <div className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-sm shadow-sm ring-1 ring-neutral-200">
              ...
              <span className="font-medium">{monthLabel(month, year)}</span>
        ...
        <div className="flex items-start gap-3">
          ...
          <SnapshotPicker year={year} month={month} currentReportId={currentReportId} options={snapshots} />
          <UploadExcel compact />
        </div>
      </div>
          ...
          {metricKey === "repeat_visit_rate" ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-2xl border bg-white p-4 shadow-sm ring-1 ring-indigo-100">
                ...
                <div className="mt-1 text-3xl font-semibold text-indigo-700">{formatMetric(metricKey, mainAnimated)}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-2xl border bg-white p-4 shadow-sm ring-1 ring-indigo-100">
                <div className="flex items-center justify-between gap-3">
                  <div className="text-sm text-neutral-500">Доля ушедших клиентов</div>
                  {compareActive && <DeltaBadge metricKey={metricKey} delta={mainDelta} />}
                </div>
                <div className="mt-1 text-3xl font-semibold text-indigo-700">{formatMetric(metricKey, mainAnimated)}</div>
                <div className="mt-2 text-sm text-neutral-600">
                  {compareActive && bOverall ? (
                    ...
                    <>
                      Пост. прошл: <span className="font-medium">{aCh.prev}</span>
                      {" • "}
                      Пост. тек: <span className="font-medium">{aCh.curr}</span>
                      {" • "}
                      Ушли: <span className="font-medium">{aCh.left}</span>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="text-sm text-neutral-500">Постоянные (прошлый месяц)</div>
                <div className="mt-1 text-3xl font-semibold">{aCh.prev}</div>
                {compareActive && bOverall && (
                  <div className="mt-2 text-sm text-neutral-600">
                    B: <span className="font-medium">{bCh.prev}</span>
                  </div>
                )}
              </div>

              <div className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="text-sm text-neutral-500">Постоянные (текущий месяц)</div>
                <div className="mt-1 text-3xl font-semibold">{aCh.curr}</div>
                {compareActive && bOverall && (
                  <div className="mt-2 text-sm text-neutral-600">
                    B: <span className="font-medium">{bCh.curr}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="rounded-2xl border bg-white p-4 shadow-sm">
                ...
                <div className="mt-1 text-3xl font-semibold">{Math.round(aOverall!.numerator)}</div>
                {compareActive && bOverall && (
                  <div className="mt-2 text-sm text-neutral-600">
                    B: <span className="font-medium">{Math.round(bOverall.numerator)}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="px-4 py-3 border-b bg-neutral-50 flex items-center justify-between gap-4">
              ...
              {compareActive && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    <option value="drops">
                      {metricKey === "churn_rate" ? "По Δ: ухудшения" : "По Δ: просадки"}
                    </option>
                    <option value="growth">
                      {metricKey === "churn_rate" ? "По Δ: улучшения" : "По Δ: рост"}
                    </option>
                  </select>
                </div>
              )}
                ...
                <thead className="text-left text-neutral-600">
                  {!compareActive ? (
                    metricKey === "repeat_visit_rate" ? (
                      ...
                      <tr className="border-b bg-white">
                        <th className="py-3 px-4 font-medium">Специализация</th>
                        <th className="py-3 px-4 font-medium">Пост. прошл</th>
                        <th className="py-3 px-4 font-medium">Пост. тек</th>
                        <th className="py-3 px-4 font-medium">Ушли</th>
                        <th className="py-3 px-4 font-medium">Доля ушедших</th>
                      </tr>
                    ) : (
                      <tr className="border-b bg-white">
                        <th className="py-3 px-4 font-medium">Специализация</th>
                    ...
                    <>
                      <tr className="border-b bg-white">
                        ...
                        <th
                          ...
                          colSpan={metricKey === "repeat_visit_rate" ? 4 : 4}
                        >
                          A • {monthLabel(month, year)}
                        </th>
                        <th
                          className="py-3 px-4 font-medium text-neutral-500"
                          colSpan={metricKey === "repeat_visit_rate" ? 4 : 4}
                      ...
                      {metricKey === "repeat_visit_rate" ? (
                        ...
                        <tr className="border-b bg-white">
                          <th className="py-2 px-4 font-medium">Пост. прошл</th>
                          <th className="py-2 px-4 font-medium">Пост. тек</th>
                          <th className="py-2 px-4 font-medium">Ушли</th>
                          <th className="py-2 px-4 font-medium">Доля</th>
                          <th className="py-2 px-4 font-medium">Пост. прошл</th>
                          <th className="py-2 px-4 font-medium">Пост. тек</th>
                          <th className="py-2 px-4 font-medium">Ушли</th>
                          <th className="py-2 px-4 font-medium">Доля</th>
                        ...
                        <tr className="border-b bg-white">
                          <th className="py-2 px-4 font-medium">Все</th>
                          <th className="py-2 px-4 font-medium">Повторные</th>
                          <th className="py-2 px-4 font-medium">Возвр.</th>
                ...
                <tbody>
                  {!compareActive ? (
                    categories.map((row) => {
                      ...
                      if (metricKey === "churn_rate") {
                        const prev = Math.max(0, Math.round(row.denominator));
                        const left = Math.max(0, Math.round(row.numerator));
                        const curr = Math.max(0, prev - left);
                        return (
                          <tr key={row.id} className="border-b last:border-b-0 hover:bg-neutral-50">
                            <td className="py-3 px-4">{name}</td>
                            <td className="py-3 px-4">{prev}</td>
                            <td className="py-3 px-4">{curr}</td>
                    ...
                    sortedTable.map((r) => {
                      ...
                      const deltaCell = <DeltaBadge metricKey={metricKey} delta={delta} />;
                      ...
                      if (metricKey === "repeat_visit_rate") {
                        ...
                        return (
                          <tr key={r.key} className={cn("border-b last:border-b-0 hover:bg-neutral-50", rowToneClass)}>
                      ...
                      if (metricKey === "churn_rate") {
                        const aPrev = a ? Math.max(0, Math.round(a.denominator)) : null;
                        const aLeft = a ? Math.max(0, Math.round(a.numerator)) : null;
                        const aCurr = aPrev != null && aLeft != null ? Math.max(0, aPrev - aLeft) : null;

                        const bPrev = bb ? Math.max(0, Math.round(bb.denominator)) : null;
                        const bLeft = bb ? Math.max(0, Math.round(bb.numerator)) : null;
                        const bCurr = bPrev != null && bLeft != null ? Math.max(0, bPrev - bLeft) : null;

                        return (
                          <tr key={r.key} className={cn("border-b last:border-b-0 hover:bg-neutral-50", rowToneClass)}>
                            <td className="py-3 px-4">{r.name}</td>

                            <td className="py-3 px-4">{aPrev == null ? "—" : aPrev}</td>
                            <td className="py-3 px-4">{aCurr == null ? "—" : aCurr}</td>
                            <td className="py-3 px-4">{aLeft == null ? "—" : aLeft}</td>
                            <td className="py-3 px-4 font-medium">{a ? `${a.value.toFixed(2)}%` : "—"}</td>

                            <td className="py-3 px-4">{bPrev == null ? "—" : bPrev}</td>
                            <td className="py-3 px-4">{bCurr == null ? "—" : bCurr}</td>
                            <td className="py-3 px-4">{bLeft == null ? "—" : bLeft}</td>
                            <td className="py-3 px-4 font-medium">{bb ? `${bb.value.toFixed(2)}%` : "—"}</td>
                  ...
                  {!compareActive && categories.length === 0 && (
                    <tr>
                      <td
                        className="py-6 px-4 text-neutral-600"
                        colSpan={metricKey === "repeat_visit_rate" || metricKey === "churn_rate" ? 5 : 4}
                      >
                  ...
                  {compareActive && sortedTable.length === 0 && (
                    <tr>
                      <td className="py-6 px-4 text-neutral-600" colSpan={10}>
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="Сравнить период">
        <div className="space-y-2">
          ...
          <div className="rounded-xl border bg-neutral-50 px-3 py-2 text-sm">{monthLabel(month, year)}</div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="text-xs text-neutral-500">Доступны только те месяцы, по которым уже есть данные в истории.</div>
        </div>

        <div className="flex items-center justify-between gap-3 pt-2">
          <button onClick={onClearCompare} className="rounded-lg px-3 py-2 text-sm text-neutral-700 hover:bg-neutral-100">
            Сбросить
          </button>
          <button
