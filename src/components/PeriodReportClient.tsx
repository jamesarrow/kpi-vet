...
function deltaTone(d: number) {
  if (d > 0.00001) return "pos";
  if (d < -0.00001) return "neg";
  return "neu";
}
...
export function PeriodReportClient({
  ...
  const mainDelta = aOverall && bOverall ? mainB - mainA : null;
  const mainAnimated = useAnimatedNumber(compareActive && bOverall ? mainB : mainA || 0);
  ...
  const table = useMemo(() => {
    ...
    return keys
      .map((k) => {
        ...
        const delta = a && bb ? bb.value - a.value : null;
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-3">
          <div className={tabWrap}>
            ...
            <span
              className={cn(tabBase, "text-neutral-400 cursor-not-allowed select-none")}
          ...
          {compareActive && (
            <div className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-sm shadow-sm ring-1 ring-neutral-200">
              ...
              <span className="font-medium">M{month}.{year}</span>
              <span className="text-neutral-400">vs</span>
              <span className="font-medium">M{compareMonth0}.{compareYear0}</span>
              <button
            ...
            <div className="px-4 py-3 border-b bg-neutral-50 flex items-center justify-between gap-4">
              ...
              {compareActive && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    <option value="drops">По Δ: просадки</option>
                    <option value="growth">По Δ: рост</option>
                  </select>
                </div>
              )}
            </div>
                  ...
                  {!compareActive ? (
                    ...
                    <>
                      <tr className="border-b bg-white">
                        ...
                        <th className="py-3 px-4 font-medium text-neutral-500" colSpan={metricKey === "repeat_visit_rate" ? 4 : 3}>
                          B • M{compareMonth0}.{compareYear0}
                        </th>
                        <th className="py-3 px-4 font-medium" rowSpan={2}>
                          Δ
                        </th>
                ...
                <tbody>
                  {!compareActive ? (
                    ...
                    sortedTable.map((r) => {
                      ...
                      const bb = r.b;
                      const delta = r.delta;
                      const tone = delta == null ? "neu" : deltaTone(delta);
                      const deltaCell = (
